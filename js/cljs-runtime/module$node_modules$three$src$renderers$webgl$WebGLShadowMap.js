...
_objects,_capabilities){function getDepthMaterial(object,material,light,shadowCameraNear,shadowCameraFar,type){object=!0===light.isPointLight?object.customDistanceMaterial:object.customDepthMaterial;object=void 0!==object?object:!0===light.isPointLight?_distanceMaterial:_depthMaterial;if(_renderer.localClippingEnabled&&!0===material.clipShadows&&Array.isArray(material.clippingPlanes)&&0!==material.clippingPlanes.length||material.displacementMap&&0!==material.displacementScale||material.alphaMap&&
0<material.alphaTest){var keyA=object.uuid;const keyB=material.uuid;let materialsForVariant=_materialCache[keyA];void 0===materialsForVariant&&(materialsForVariant={},_materialCache[keyA]=materialsForVariant);keyA=materialsForVariant[keyB];void 0===keyA&&(keyA=object.clone(),materialsForVariant[keyB]=keyA);object=keyA}object.visible=material.visible;object.wireframe=material.wireframe;object.side=type===_constants.VSMShadowMap?null!==material.shadowSide?material.shadowSide:material.side:null!==material.shadowSide?
...
_viewportSize.y*shadowFrameExtents.y,shadow.mapSize.y=_viewportSize.y);null===shadow.map&&(shadow.map=new _WebGLRenderTarget.WebGLRenderTarget(_shadowMapSize.x,_shadowMapSize.y,this.type!==_constants.VSMShadowMap?{minFilter:_constants.NearestFilter,magFilter:_constants.NearestFilter}:{}),shadow.map.texture.name=light.name+".shadowMap",shadow.camera.updateProjectionMatrix());_renderer.setRenderTarget(shadow.map);_renderer.clear();shadowFrameExtents=shadow.getViewportCount();for(var vp=0;vp<shadowFrameExtents;vp++){const viewport=
shadow.getViewport(vp);_viewport.set(_viewportSize.x*viewport.x,_viewportSize.y*viewport.y,_viewportSize.x*viewport.z,_viewportSize.y*viewport.w);_state.viewport(_viewport);shadow.updateMatrices(light,vp);_frustum=shadow.getFrustum();renderObject(scene,camera,shadow.camera,light,this.type)}!0!==shadow.isPointLightShadow&&this.type===_constants.VSMShadowMap&&(light=shadow,shadowFrameExtents=camera,vp=_objects.update(fullScreenMesh),shadowMaterialVertical.defines.VSM_SAMPLES!==light.blurSamples&&(shadowMaterialVertical.defines.VSM_SAMPLES=
light.blurSamples,shadowMaterialHorizontal.defines.VSM_SAMPLES=light.blurSamples,shadowMaterialVertical.needsUpdate=!0,shadowMaterialHorizontal.needsUpdate=!0),null===light.mapPass&&(light.mapPass=new _WebGLRenderTarget.WebGLRenderTarget(_shadowMapSize.x,_shadowMapSize.y)),shadowMaterialVertical.uniforms.shadow_pass.value=light.map.texture,shadowMaterialVertical.uniforms.resolution.value=light.mapSize,shadowMaterialVertical.uniforms.radius.value=light.radius,_renderer.setRenderTarget(light.mapPass),
_renderer.clear(),_renderer.renderBufferDirect(shadowFrameExtents,null,vp,shadowMaterialVertical,fullScreenMesh,null),shadowMaterialHorizontal.uniforms.shadow_pass.value=light.mapPass.texture,shadowMaterialHorizontal.uniforms.resolution.value=light.mapSize,shadowMaterialHorizontal.uniforms.radius.value=light.radius,_renderer.setRenderTarget(light.map),_renderer.clear(),_renderer.renderBufferDirect(shadowFrameExtents,null,vp,shadowMaterialHorizontal,fullScreenMesh,null));shadow.needsUpdate=!1}}scope.needsUpdate=
!1;_renderer.setRenderTarget(currentRenderTarget,activeCubeFace,activeMipmapLevel)}}};var _constants=require("module$node_modules$three$src$constants"),_WebGLRenderTarget=require("module$node_modules$three$src$renderers$WebGLRenderTarget"),_MeshDepthMaterial=require("module$node_modules$three$src$materials$MeshDepthMaterial"),_MeshDistanceMaterial=require("module$node_modules$three$src$materials$MeshDistanceMaterial"),_ShaderMaterial=require("module$node_modules$three$src$materials$ShaderMaterial"),
_BufferAttribute=require("module$node_modules$three$src$core$BufferAttribute"),_BufferGeometry=require("module$node_modules$three$src$core$BufferGeometry"),_Mesh=require("module$node_modules$three$src$objects$Mesh"),_Vector=require("module$node_modules$three$src$math$Vector4"),_Vector2=require("module$node_modules$three$src$math$Vector2"),_Frustum=require("module$node_modules$three$src$math$Frustum"),vsm=function(obj,nodeInterop){if(!nodeInterop&&obj&&obj.__esModule)return obj;if(null===obj||"object"!==
...
obj;nodeInterop&&nodeInterop.set(obj,newObj);return newObj}(require("module$node_modules$three$src$renderers$shaders$ShaderLib$vsm_glsl"))}
