...
forward){return extendSel(view,range=>view.moveVertically(range,forward,Math.max(view.defaultLineHeight,Math.min(view.dom.clientHeight,innerHeight)-5)))}function deleteBy({state:state$1,dispatch},by){if(state$1.readOnly)return!1;let event="delete.selection",changes=state$1.changeByRange(range=>{let {from,to}=range;if(from==to){let towards=by(from);towards<from?event="delete.backward":towards>from&&(event="delete.forward");from=Math.min(from,towards);to=Math.max(to,towards)}return from==to?{range}:
{changes:{from,to},range:state$jscomp$0.EditorSelection.cursor(from)}});if(changes.changes.empty)return!1;dispatch(state$1.update(changes,{scrollIntoView:!0,userEvent:event,effects:"delete.selection"==event?view$jscomp$0.EditorView.announce.of(state$1.phrase("Selection deleted")):void 0}));return!0}function skipAtomic(target,pos,forward){if(target instanceof view$jscomp$0.EditorView)for(let ranges of target.state.facet(view$jscomp$0.EditorView.atomicRanges).map(f=>f(target)))ranges.between(pos,pos,
(from,to)=>{from<pos&&to>pos&&(pos=forward?to:from)});return pos}function selectedLineBlocks(state){let blocks=[];var upto=-1;for(let range of state.selection.ranges){let startLine=state.doc.lineAt(range.from),endLine=state.doc.lineAt(range.to);range.empty||range.to!=endLine.from||(endLine=state.doc.lineAt(range.to-1));upto>=startLine.number?(upto=blocks[blocks.length-1],upto.to=endLine.to,upto.ranges.push(range)):blocks.push({from:startLine.from,to:endLine.to,ranges:[range]});upto=endLine.number+
...
1),pos.mapPos(range.head,1))}})}Object.defineProperty(exports,"__esModule",{value:!0});var state$jscomp$0=require("module$node_modules$$codemirror$state$dist$index_cjs"),view$jscomp$0=require("module$node_modules$$codemirror$view$dist$index_cjs"),language=require("module$node_modules$$codemirror$language$dist$index_cjs"),common=require("module$node_modules$$lezer$common$dist$index_cjs");const toggleComment=target=>{let config=getConfig(target.state);return config.line?toggleLineComment(target):config.block?
toggleBlockCommentByLine(target):!1},toggleLineComment=command$jscomp$0(changeLineComment,0),lineComment=command$jscomp$0(changeLineComment,1),lineUncomment=command$jscomp$0(changeLineComment,2),toggleBlockComment=command$jscomp$0(changeBlockComment,0),blockComment=command$jscomp$0(changeBlockComment,1),blockUncomment=command$jscomp$0(changeBlockComment,2),toggleBlockCommentByLine=command$jscomp$0((o,s)=>{let ranges=[];for(let r of s.selection.ranges){let fromLine=s.doc.lineAt(r.from),toLine=r.to<=
fromLine.to?fromLine:s.doc.lineAt(r.to),last=ranges.length-1;0<=last&&ranges[last].to>fromLine.from?ranges[last].to=toLine.to:ranges.push({from:fromLine.from,to:toLine.to})}return changeBlockComment(o,s,ranges)},0),fromHistory=state$jscomp$0.Annotation.define(),isolateHistory=state$jscomp$0.Annotation.define(),invertedEffects=state$jscomp$0.Facet.define(),historyConfig=state$jscomp$0.Facet.define({combine(configs){return state$jscomp$0.combineConfig(configs,{minDepth:100,newGroupDelay:500},{minDepth:Math.max,
newGroupDelay:Math.min})}}),historyField_=state$jscomp$0.StateField.define({create(){return HistoryState.empty},update(state$1,tr){let config=tr.state.facet(historyConfig);var fromHist=tr.annotation(fromHistory);if(fromHist){var selection=tr.docChanged?state$jscomp$0.EditorSelection.single(changeEnd(tr.changes)):void 0;selection=HistEvent.fromTransaction(tr,selection);var from=fromHist.side;state$1=0==from?state$1.undone:state$1.done;state$1=selection?updateBranch(state$1,state$1.length,config.minDepth,
selection):addSelection(state$1,tr.startState.selection);return new HistoryState(0==from?fromHist.rest:state$1,0==from?state$1:fromHist.rest)}fromHist=tr.annotation(isolateHistory);if("full"==fromHist||"before"==fromHist)state$1=state$1.isolate();if(!1===tr.annotation(state$jscomp$0.Transaction.addToHistory))return tr.changes.empty?state$1:state$1.addMapping(tr.changes.desc);selection=HistEvent.fromTransaction(tr);from=tr.annotation(state$jscomp$0.Transaction.time);let userEvent=tr.annotation(state$jscomp$0.Transaction.userEvent);
selection?state$1=state$1.addChanges(selection,from,userEvent,config.newGroupDelay,config.minDepth):tr.selection&&(state$1=state$1.addSelection(tr.startState.selection,from,userEvent,config.newGroupDelay));if("full"==fromHist||"after"==fromHist)state$1=state$1.isolate();return state$1},toJSON(value){return{done:value.done.map(e=>e.toJSON()),undone:value.undone.map(e=>e.toJSON())}},fromJSON(json){return new HistoryState(json.done.map(HistEvent.fromJSON),json.undone.map(HistEvent.fromJSON))}}),historyField=
historyField_,undo=cmd(0,!1),redo=cmd(1,!1),undoSelection=cmd(0,!0),redoSelection=cmd(1,!0),undoDepth=depth(0),redoDepth=depth(1);class HistEvent{constructor(changes,effects,mapped,startSelection,selectionsAfter){this.changes=changes;this.effects=effects;this.mapped=mapped;this.startSelection=startSelection;this.selectionsAfter=selectionsAfter}setSelAfter(after){return new HistEvent(this.changes,this.effects,this.mapped,this.startSelection,after)}toJSON(){var _a,_b,_c;return{changes:null===(_a=this.changes)||
void 0===_a?void 0:_a.toJSON(),mapped:null===(_b=this.mapped)||void 0===_b?void 0:_b.toJSON(),startSelection:null===(_c=this.startSelection)||void 0===_c?void 0:_c.toJSON(),selectionsAfter:this.selectionsAfter.map(s=>s.toJSON())}}static fromJSON(json){return new HistEvent(json.changes&&state$jscomp$0.ChangeSet.fromJSON(json.changes),[],json.mapped&&state$jscomp$0.ChangeDesc.fromJSON(json.mapped),json.startSelection&&state$jscomp$0.EditorSelection.fromJSON(json.startSelection),json.selectionsAfter.map(state$jscomp$0.EditorSelection.fromJSON))}static fromTransaction(tr,
selection){let effects=none;for(let invert of tr.startState.facet(invertedEffects)){let result=invert(tr);result.length&&(effects=effects.concat(result))}return!effects.length&&tr.changes.empty?null:new HistEvent(tr.changes.invert(tr.startState.doc),effects,void 0,selection||tr.startState.selection,none)}static selection(selections){return new HistEvent(void 0,none,void 0,void 0,selections)}}const none=[],joinableUserEvent=/^(input\.type|delete)($|\.)/;class HistoryState{constructor(done,undone,prevTime=
0,prevUserEvent){this.done=done;this.undone=undone;this.prevTime=prevTime;this.prevUserEvent=prevUserEvent}isolate(){return this.prevTime?new HistoryState(this.done,this.undone):this}addChanges(event,time,userEvent,newGroupDelay,maxLen){let done=this.done,lastEvent=done[done.length-1];done=lastEvent&&lastEvent.changes&&!lastEvent.changes.empty&&event.changes&&(!userEvent||joinableUserEvent.test(userEvent))&&(!lastEvent.selectionsAfter.length&&time-this.prevTime<newGroupDelay&&isAdjacent(lastEvent.changes,
event.changes)||"input.type.compose"==userEvent)?updateBranch(done,done.length-1,maxLen,new HistEvent(event.changes.compose(lastEvent.changes),conc(event.effects,lastEvent.effects),lastEvent.mapped,lastEvent.startSelection,none)):updateBranch(done,done.length,maxLen,event);return new HistoryState(done,none,time,userEvent)}addSelection(selection,time,userEvent,newGroupDelay){let last=this.done.length?this.done[this.done.length-1].selectionsAfter:none;return 0<last.length&&time-this.prevTime<newGroupDelay&&
userEvent==this.prevUserEvent&&userEvent&&/^select($|\.)/.test(userEvent)&&eqSelectionShape(last[last.length-1],selection)?this:new HistoryState(addSelection(this.done,selection),this.undone,time,userEvent)}addMapping(mapping){return new HistoryState(addMappingToBranch(this.done,mapping),addMappingToBranch(this.undone,mapping),this.prevTime,this.prevUserEvent)}pop(side,state,selection){var branch=0==side?this.done:this.undone;if(0==branch.length)return null;var event=branch[branch.length-1];if(selection&&
event.selectionsAfter.length){selection=state.update;event=event.selectionsAfter[event.selectionsAfter.length-1];var JSCompiler_temp_const=fromHistory.of;let last=branch[branch.length-1],newBranch=branch.slice();newBranch[branch.length-1]=last.setSelAfter(last.selectionsAfter.slice(0,last.selectionsAfter.length-1));return selection.call(state,{selection:event,annotations:JSCompiler_temp_const.call(fromHistory,{side,rest:newBranch}),userEvent:0==side?"select.undo":"select.redo",scrollIntoView:!0})}return event.changes?
(branch=1==branch.length?none:branch.slice(0,branch.length-1),event.mapped&&(branch=addMappingToBranch(branch,event.mapped)),state.update({changes:event.changes,selection:event.startSelection,effects:event.effects,annotations:fromHistory.of({side,rest:branch}),filter:!1,userEvent:0==side?"undo":"redo",scrollIntoView:!0})):null}}HistoryState.empty=new HistoryState(none,none);const historyKeymap=[{key:"Mod-z",run:undo,preventDefault:!0},{key:"Mod-y",mac:"Mod-Shift-z",run:redo,preventDefault:!0},{linux:"Ctrl-Shift-z",
run:redo,preventDefault:!0},{key:"Mod-u",run:undoSelection,preventDefault:!0},{key:"Alt-u",mac:"Mod-Shift-u",run:redoSelection,preventDefault:!0}],cursorCharLeft=view=>cursorByChar(view,!ltrAtCursor(view)),cursorCharRight=view=>cursorByChar(view,ltrAtCursor(view)),cursorGroupLeft=view=>cursorByGroup(view,!ltrAtCursor(view)),cursorGroupRight=view=>cursorByGroup(view,ltrAtCursor(view)),cursorSyntaxLeft=view=>moveSel(view,range=>moveBySyntax(view.state,range,!ltrAtCursor(view))),cursorSyntaxRight=view=>
moveSel(view,range=>moveBySyntax(view.state,range,ltrAtCursor(view))),cursorLineUp=view=>cursorByLine(view,!1),cursorLineDown=view=>cursorByLine(view,!0),cursorPageUp=view=>cursorByPage(view,!1),cursorPageDown=view=>cursorByPage(view,!0),cursorLineBoundaryForward=view=>moveSel(view,range=>moveByLineBoundary(view,range,!0)),cursorLineBoundaryBackward=view=>moveSel(view,range=>moveByLineBoundary(view,range,!1)),cursorLineBoundaryLeft=view=>moveSel(view,range=>moveByLineBoundary(view,range,!ltrAtCursor(view))),
cursorLineBoundaryRight=view=>moveSel(view,range=>moveByLineBoundary(view,range,ltrAtCursor(view))),cursorLineStart=view=>moveSel(view,range=>state$jscomp$0.EditorSelection.cursor(view.lineBlockAt(range.head).from,1)),cursorLineEnd=view=>moveSel(view,range=>state$jscomp$0.EditorSelection.cursor(view.lineBlockAt(range.head).to,-1)),cursorMatchingBracket=({state,dispatch})=>toMatchingBracket(state,dispatch,!1),selectCharLeft=view=>selectByChar(view,!ltrAtCursor(view)),selectCharRight=view=>selectByChar(view,
ltrAtCursor(view)),selectGroupLeft=view=>selectByGroup(view,!ltrAtCursor(view)),selectGroupRight=view=>selectByGroup(view,ltrAtCursor(view)),selectSyntaxLeft=view=>extendSel(view,range=>moveBySyntax(view.state,range,!ltrAtCursor(view))),selectSyntaxRight=view=>extendSel(view,range=>moveBySyntax(view.state,range,ltrAtCursor(view))),selectLineUp=view=>selectByLine(view,!1),selectLineDown=view=>selectByLine(view,!0),selectPageUp=view=>selectByPage(view,!1),selectPageDown=view=>selectByPage(view,!0),
selectLineBoundaryForward=view=>extendSel(view,range=>moveByLineBoundary(view,range,!0)),selectLineBoundaryBackward=view=>extendSel(view,range=>moveByLineBoundary(view,range,!1)),selectLineBoundaryLeft=view=>extendSel(view,range=>moveByLineBoundary(view,range,!ltrAtCursor(view))),selectLineBoundaryRight=view=>extendSel(view,range=>moveByLineBoundary(view,range,ltrAtCursor(view))),selectLineStart=view=>extendSel(view,range=>state$jscomp$0.EditorSelection.cursor(view.lineBlockAt(range.head).from)),
selectLineEnd=view=>extendSel(view,range=>state$jscomp$0.EditorSelection.cursor(view.lineBlockAt(range.head).to)),cursorDocStart=({state,dispatch})=>{dispatch(setSel(state,{anchor:0}));return!0},cursorDocEnd=({state,dispatch})=>{dispatch(setSel(state,{anchor:state.doc.length}));return!0},selectDocStart=({state,dispatch})=>{dispatch(setSel(state,{anchor:state.selection.main.anchor,head:0}));return!0},selectDocEnd=({state,dispatch})=>{dispatch(setSel(state,{anchor:state.selection.main.anchor,head:state.doc.length}));
...
range=>{var _a;let context=language.syntaxTree(state$1).resolveInner(range.head,1);for(;!(context.from<range.from&&context.to>=range.to||context.to>range.to&&context.from<=range.from)&&(null===(_a=context.parent)||void 0===_a?0:_a.parent);)context=context.parent;return state$jscomp$0.EditorSelection.range(context.to,context.from)});dispatch(setSel(state$1,selection));return!0},simplifySelection=({state:state$1,dispatch})=>{let cur=state$1.selection,selection=null;1<cur.ranges.length?selection=state$jscomp$0.EditorSelection.create([cur.main]):
cur.main.empty||(selection=state$jscomp$0.EditorSelection.create([state$jscomp$0.EditorSelection.cursor(cur.main.head)]));if(!selection)return!1;dispatch(setSel(state$1,selection));return!0},deleteByChar=(target,forward)=>deleteBy(target,pos=>{var {state:state$1}=target,line=state$1.doc.lineAt(pos),before;if(!forward&&pos>line.from&&pos<line.from+200&&!/[^ \t]/.test(before=line.text.slice(0,pos-line.from))){if("\t"==before[before.length-1])return pos-1;state$1=state$jscomp$0.countColumn(before,state$1.tabSize)%
language.getIndentUnit(state$1)||language.getIndentUnit(state$1);for(line=0;line<state$1&&" "==before[before.length-1-line];line++)pos--;before=pos}else before=state$jscomp$0.findClusterBreak(line.text,pos-line.from,forward,forward)+line.from,before==pos&&line.number!=(forward?state$1.doc.lines:1)&&(before+=forward?1:-1);return skipAtomic(target,before,forward)}),deleteCharBackward=view=>deleteByChar(view,!1),deleteCharForward=view=>deleteByChar(view,!0),deleteByGroup=(target,forward)=>deleteBy(target,
start=>{let pos=start,{state:state$1}=target,line=state$1.doc.lineAt(pos),categorize=state$1.charCategorizer(pos);for(let cat=null;;){if(pos==(forward?line.to:line.from)){pos==start&&line.number!=(forward?state$1.doc.lines:1)&&(pos+=forward?1:-1);break}let next=state$jscomp$0.findClusterBreak(line.text,pos-line.from,forward)+line.from,nextChar=line.text.slice(Math.min(pos,next)-line.from,Math.max(pos,next)-line.from),nextCat=categorize(nextChar);if(null!=cat&&nextCat!=cat)break;if(" "!=nextChar||
pos!=start)cat=nextCat;pos=next}return skipAtomic(target,pos,forward)}),deleteGroupBackward=target=>deleteByGroup(target,!1),deleteGroupForward=target=>deleteByGroup(target,!0),deleteToLineEnd=view=>deleteBy(view,pos=>{let lineEnd=view.lineBlockAt(pos).to;return skipAtomic(view,pos<lineEnd?lineEnd:Math.min(view.state.doc.length,pos+1),!0)}),deleteToLineStart=view=>deleteBy(view,pos=>{let lineStart=view.lineBlockAt(pos).from;return skipAtomic(view,pos>lineStart?lineStart:Math.max(0,pos-1),!1)}),splitLine=
({state:state$1,dispatch})=>{if(state$1.readOnly)return!1;let changes=state$1.changeByRange(range=>({changes:{from:range.from,to:range.to,insert:state$jscomp$0.Text.of(["",""])},range:state$jscomp$0.EditorSelection.cursor(range.from)}));dispatch(state$1.update(changes,{scrollIntoView:!0,userEvent:"input"}));return!0},transposeChars=({state:state$1,dispatch})=>{if(state$1.readOnly)return!1;let changes=state$1.changeByRange(range=>{if(!range.empty||0==range.from||range.from==state$1.doc.length)return{range};
range=range.from;var line=state$1.doc.lineAt(range);let from=range==line.from?range-1:state$jscomp$0.findClusterBreak(line.text,range-line.from,!1)+line.from;line=range==line.to?range+1:state$jscomp$0.findClusterBreak(line.text,range-line.from,!0)+line.from;return{changes:{from,to:line,insert:state$1.doc.slice(range,line).append(state$1.doc.slice(from,range))},range:state$jscomp$0.EditorSelection.cursor(line)}});if(changes.changes.empty)return!1;dispatch(state$1.update(changes,{scrollIntoView:!0,
userEvent:"move.character"}));return!0},moveLineUp=({state,dispatch})=>moveLine(state,dispatch,!1),moveLineDown=({state,dispatch})=>moveLine(state,dispatch,!0),copyLineUp=({state,dispatch})=>copyLine(state,dispatch,!1),copyLineDown=({state,dispatch})=>copyLine(state,dispatch,!0),deleteLine=view=>{if(view.state.readOnly)return!1;let {state}=view,changes=state.changes(selectedLineBlocks(state).map(({from,to})=>{0<from?from--:to<state.doc.length&&to++;return{from,to}})),selection=updateSel(state.selection,
range=>view.moveVertically(range,!0)).map(changes);view.dispatch({changes,selection,scrollIntoView:!0,userEvent:"delete.line"});return!0},insertNewlineAndIndent=newlineAndIndent(!1),insertBlankLine=newlineAndIndent(!0),indentSelection=({state,dispatch})=>{if(state.readOnly)return!1;let updated=Object.create(null),context=new language.IndentContext(state,{overrideIndentation:start=>{start=updated[start];return null==start?-1:start}}),changes$jscomp$0=changeBySelectedLine(state,(line,changes,range)=>
{let indent=language.getIndentation(context,line.from);if(null!=indent){/\S/.test(line.text)||(indent=0);var cur=/^\s*/.exec(line.text)[0],norm=language.indentString(state,indent);if(cur!=norm||range.from<line.from+cur.length)updated[line.from]=indent,changes.push({from:line.from,to:line.from+cur.length,insert:norm})}});changes$jscomp$0.changes.empty||dispatch(state.update(changes$jscomp$0,{userEvent:"indent"}));return!0},indentMore=({state,dispatch})=>{if(state.readOnly)return!1;dispatch(state.update(changeBySelectedLine(state,
(line,changes)=>{changes.push({from:line.from,insert:state.facet(language.indentUnit)})}),{userEvent:"input.indent"}));return!0},indentLess=({state:state$1,dispatch})=>{if(state$1.readOnly)return!1;dispatch(state$1.update(changeBySelectedLine(state$1,(line,changes)=>{let space=/^\s*/.exec(line.text)[0];if(space){var col=state$jscomp$0.countColumn(space,state$1.tabSize),keep=0;for(col=language.indentString(state$1,Math.max(0,col-language.getIndentUnit(state$1)));keep<space.length&&keep<col.length&&
space.charCodeAt(keep)==col.charCodeAt(keep);)keep++;changes.push({from:line.from+keep,to:line.from+space.length,insert:col.slice(keep)})}}),{userEvent:"delete.dedent"}));return!0},emacsStyleKeymap=[{key:"Ctrl-b",run:cursorCharLeft,shift:selectCharLeft,preventDefault:!0},{key:"Ctrl-f",run:cursorCharRight,shift:selectCharRight},{key:"Ctrl-p",run:cursorLineUp,shift:selectLineUp},{key:"Ctrl-n",run:cursorLineDown,shift:selectLineDown},{key:"Ctrl-a",run:cursorLineStart,shift:selectLineStart},{key:"Ctrl-e",
run:cursorLineEnd,shift:selectLineEnd},{key:"Ctrl-d",run:deleteCharForward},{key:"Ctrl-h",run:deleteCharBackward},{key:"Ctrl-k",run:deleteToLineEnd},{key:"Ctrl-Alt-h",run:deleteGroupBackward},{key:"Ctrl-o",run:splitLine},{key:"Ctrl-t",run:transposeChars},{key:"Ctrl-v",run:cursorPageDown}],standardKeymap=[{key:"ArrowLeft",run:cursorCharLeft,shift:selectCharLeft,preventDefault:!0},{key:"Mod-ArrowLeft",mac:"Alt-ArrowLeft",run:cursorGroupLeft,shift:selectGroupLeft,preventDefault:!0},{mac:"Cmd-ArrowLeft",
run:cursorLineBoundaryLeft,shift:selectLineBoundaryLeft,preventDefault:!0},{key:"ArrowRight",run:cursorCharRight,shift:selectCharRight,preventDefault:!0},{key:"Mod-ArrowRight",mac:"Alt-ArrowRight",run:cursorGroupRight,shift:selectGroupRight,preventDefault:!0},{mac:"Cmd-ArrowRight",run:cursorLineBoundaryRight,shift:selectLineBoundaryRight,preventDefault:!0},{key:"ArrowUp",run:cursorLineUp,shift:selectLineUp,preventDefault:!0},{mac:"Cmd-ArrowUp",run:cursorDocStart,shift:selectDocStart},{mac:"Ctrl-ArrowUp",
run:cursorPageUp,shift:selectPageUp},{key:"ArrowDown",run:cursorLineDown,shift:selectLineDown,preventDefault:!0},{mac:"Cmd-ArrowDown",run:cursorDocEnd,shift:selectDocEnd},{mac:"Ctrl-ArrowDown",run:cursorPageDown,shift:selectPageDown},{key:"PageUp",run:cursorPageUp,shift:selectPageUp},{key:"PageDown",run:cursorPageDown,shift:selectPageDown},{key:"Home",run:cursorLineBoundaryBackward,shift:selectLineBoundaryBackward,preventDefault:!0},{key:"Mod-Home",run:cursorDocStart,shift:selectDocStart},{key:"End",
