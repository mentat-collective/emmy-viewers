...
new _BufferGeometry.BufferGeometry,material=new _MeshBasicMaterial.MeshBasicMaterial){super();this.isMesh=!0;this.type="Mesh";this.geometry=geometry;this.material=material;this.updateMorphTargets()}copy(source,recursive){super.copy(source,recursive);void 0!==source.morphTargetInfluences&&(this.morphTargetInfluences=source.morphTargetInfluences.slice());void 0!==source.morphTargetDictionary&&(this.morphTargetDictionary=Object.assign({},source.morphTargetDictionary));this.material=source.material;this.geometry=
source.geometry;return this}updateMorphTargets(){var morphAttributes=this.geometry.morphAttributes,keys=Object.keys(morphAttributes);if(0<keys.length&&(morphAttributes=morphAttributes[keys[0]],void 0!==morphAttributes)){this.morphTargetInfluences=[];this.morphTargetDictionary={};for(let m=0,ml=morphAttributes.length;m<ml;m++)keys=morphAttributes[m].name||String(m),this.morphTargetInfluences.push(0),this.morphTargetDictionary[keys]=m}}raycast(raycaster,intersects){var geometry=this.geometry;const material=
this.material;var matrixWorld=this.matrixWorld;if(void 0!==material&&(null===geometry.boundingSphere&&geometry.computeBoundingSphere(),_sphere.copy(geometry.boundingSphere),_sphere.applyMatrix4(matrixWorld),!1!==raycaster.ray.intersectsSphere(_sphere)&&(_inverseMatrix.copy(matrixWorld).invert(),_ray.copy(raycaster.ray).applyMatrix4(_inverseMatrix),null===geometry.boundingBox||!1!==_ray.intersectsBox(geometry.boundingBox)))){var index=geometry.index;matrixWorld=geometry.attributes.position;var morphPosition=
geometry.morphAttributes.position,morphTargetsRelative=geometry.morphTargetsRelative,uv=geometry.attributes.uv,uv2=geometry.attributes.uv2,groups=geometry.groups;geometry=geometry.drawRange;if(null!==index)if(Array.isArray(material))for(let i=0,il=groups.length;i<il;i++){var group=groups[i],groupMaterial=material[group.materialIndex];var intersection=Math.max(group.start,geometry.start);var end=Math.min(index.count,Math.min(group.start+group.count,geometry.start+geometry.count));for(let j=intersection,
jl=end;j<jl;j+=3){intersection=index.getX(j);end=index.getX(j+1);const c=index.getX(j+2);if(intersection=checkBufferGeometryIntersection(this,groupMaterial,raycaster,_ray,matrixWorld,morphPosition,morphTargetsRelative,uv,uv2,intersection,end,c))intersection.faceIndex=Math.floor(j/3),intersection.face.materialIndex=group.materialIndex,intersects.push(intersection)}}else{groups=Math.max(0,geometry.start);geometry=Math.min(index.count,geometry.start+geometry.count);for(let i=groups,il=geometry;i<il;i+=
3)if(geometry=index.getX(i),groups=index.getX(i+1),group=index.getX(i+2),intersection=checkBufferGeometryIntersection(this,material,raycaster,_ray,matrixWorld,morphPosition,morphTargetsRelative,uv,uv2,geometry,groups,group))intersection.faceIndex=Math.floor(i/3),intersects.push(intersection)}else if(void 0!==matrixWorld)if(Array.isArray(material))for(let i=0,il=groups.length;i<il;i++){index=groups[i];group=material[index.materialIndex];groupMaterial=Math.max(index.start,geometry.start);intersection=
Math.min(matrixWorld.count,Math.min(index.start+index.count,geometry.start+geometry.count));for(let j=groupMaterial,jl=intersection;j<jl;j+=3)if(intersection=checkBufferGeometryIntersection(this,group,raycaster,_ray,matrixWorld,morphPosition,morphTargetsRelative,uv,uv2,j,j+1,j+2))intersection.faceIndex=Math.floor(j/3),intersection.face.materialIndex=index.materialIndex,intersects.push(intersection)}else{index=Math.max(0,geometry.start);geometry=Math.min(matrixWorld.count,geometry.start+geometry.count);
for(let i=index,il=geometry;i<il;i+=3)if(intersection=checkBufferGeometryIntersection(this,material,raycaster,_ray,matrixWorld,morphPosition,morphTargetsRelative,uv,uv2,i,i+1,i+2))intersection.faceIndex=Math.floor(i/3),intersects.push(intersection)}}}}exports.Mesh=Mesh}
