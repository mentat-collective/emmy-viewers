...
this.center.distanceToSquared(point);target.copy(point);deltaLengthSq>this.radius*this.radius&&(target.sub(this.center).normalize(),target.multiplyScalar(this.radius).add(this.center));return target}getBoundingBox(target){if(this.isEmpty())return target.makeEmpty(),target;target.set(this.center,this.center);target.expandByScalar(this.radius);return target}applyMatrix4(matrix){this.center.applyMatrix4(matrix);this.radius*=matrix.getMaxScaleOnAxis();return this}translate(offset){this.center.add(offset);
return this}expandByPoint(point){if(this.isEmpty())return this.center.copy(point),this.radius=0,this;_v1.subVectors(point,this.center);point=_v1.lengthSq();if(point>this.radius*this.radius){point=Math.sqrt(point);const delta=.5*(point-this.radius);this.center.addScaledVector(_v1,delta/point);this.radius+=delta}return this}union(sphere){if(sphere.isEmpty())return this;if(this.isEmpty())return this.copy(sphere),this;!0===this.center.equals(sphere.center)?this.radius=Math.max(this.radius,sphere.radius):
(_v2.subVectors(sphere.center,this.center).setLength(sphere.radius),this.expandByPoint(_v1.copy(sphere.center).add(_v2)),this.expandByPoint(_v1.copy(sphere.center).sub(_v2)));return this}equals(sphere){return sphere.center.equals(this.center)&&sphere.radius===this.radius}clone(){return(new this.constructor).copy(this)}}exports.Sphere=Sphere}
